...
extension API {
    ...
    struct Error {
        ...
        static let authen = NSError(domain: ApiManager.Path.baseURL, status: HTTPStatus.unauthorized)
        static let apiKey = NSError(domain: ApiManager.Path.baseURL, status: HTTPStatus.badRequest)
        static let json = NSError(domain: NSCocoaErrorDomain, code: 3_840, message: "The operation couldn’t be completed.")
        static let cancelRequest = NSError(domain: ApiManager.Path.baseURL, code: 999, message: "Server returns no information and closes the connection.")
        static let emptyData = NSError(domain: ApiManager.Path.baseURL, code: 997, message: "Server returns no data")
        static let noResponse = NSError(status: .noResponse)
        static let invalidURL = NSError(domain: ApiManager.Path.baseURL, code: 998, message: "Cannot detect URL")
        static let emailCannotUsed = NSError(domain: ApiManager.Path.baseURL, code: 996, message: "This email cannot be used at the moment!")
        static let usernameExistence = NSError(domain: ApiManager.Path.baseURL, code: 995, message: "")
        static let upgradeRequired = NSError(domain: ApiManager.Path.baseURL.host, code: 426, message: "ForceUpdate")
        static let invalid = NSError(domain: ApiManager.Path.baseURL.host, code: 9_999, message: "Parameter has no value")
        static let timeout = NSError(domain: ApiManager.Path.baseURL.host, code: -1_001, message: "The request timeout")
        static let forbidden = NSError(domain: ApiManager.Path.baseURL.host, code: 403, message: "The request was a legal request, but the server is refusing to respond to it.")
        static let badRequest: NSError = NSError(domain: ApiManager.Path.baseURL.host, code: 400, message: "The request cannot be fulfilled due to bad syntax.")
