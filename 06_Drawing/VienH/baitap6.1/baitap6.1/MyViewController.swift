...
final class MyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let frameTop = CGRect(x: 20, y: 20, width: view.bounds.width, height: view.bounds.height / 2 - 20)
        columnChartView.frame = frameTop
        columnChartView.backgroundColor = .white
        columnChartView.transform(values: values)
        
        let pieChartView = PieChartView()
        let frameBottom = CGRect(x: 20, y: view.bounds.height / 2 + 20, width: view.bounds.width, height: view.bounds.height / 2 - 20)
