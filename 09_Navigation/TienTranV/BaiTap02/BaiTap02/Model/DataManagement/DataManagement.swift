...
final class DataManagement {
    ...
    func getUsers(fileName: String, type: String) -> [User] {
        ...
        var users: [User] = []
        for item in array {
            guard let dict = item as? NSDictionary, let userName = dict.object(forKey: Key.userName.rawValue) as? String, let password = dict.object(forKey: Key.password.rawValue) as? String else { continue }
