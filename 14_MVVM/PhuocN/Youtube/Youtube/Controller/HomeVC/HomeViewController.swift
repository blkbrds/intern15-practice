...
final class HomeViewController: BaseViewController {
    ...
    private func updateUI(with perform: Action) {
        ...
        case .changeStatus:
            switch status {
            case .grid:
                if UserDefaults.standard.value(forKey: "youtubeAccessToken") != nil {
                    ...
                    navigationItem.rightBarButtonItems?[1] = UIBarButtonItem(image: status.image, style: .plain, target: self, action: #selector(changeMode))
                }
                homeCollectionView.reloadData()
            case .row:
                if UserDefaults.standard.value(forKey: "youtubeAccessToken") != nil {
...
//            }
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        if scrollY > scrollView.contentOffset.y {
